...
sub _move_ends
{
	...
	if(@r_borders) {
	    ...
	    # Find the best way to break the mapping
            my @move_result = $self->_right_mover2($m, $target_seq, @r_borders);
	    if(@move_result == 2) {
	...
	if(@l_borders) {
	    ...
	    # Find the best way to break the mapping
	    my @move_result = $self->_left_mover2($m, $target_seq, @l_borders);
	    if(@move_result == 2) {
...
#  return the number of bases of query seq after the first border.
{
    ...
    my $q_seq_str = uc($mapping->qSeqStr_after_tPos($first_border));
	...
	next if (length($q_seq_str_after_border) < $word_size);
	    ...
	    my $t_subseq_end = $t_subseq_start + length($q_seq_str) + 14;
	    $t_subseq_end = $t_seq->length if($t_seq->length < $t_subseq_end);
	    $t_subseq_str .= uc($t_seq->subseq($t_subseq_start, $t_subseq_end));
	    # add to array of sequences
	    push @targets, { d => $b - $first_border,
			     border2 => $word_hit,
			     seq_str => $t_subseq_str };
        }
    }
    return length($q_seq_str) if (@targets == 1);

    # Align; find best target
    my ($hsps, $target) = $self->_align_and_select_best($q_seq_str, \@targets);
    return length($q_seq_str) unless ($hsps);
...
# See _right_mover for further details.
{
    ...
    my $q_seq_str = reverse uc($mapping->qSeqStr_before_tPos($first_border));
    ...
    my @targets = ({d => 0,         # distance from seq start to junction
		    ...
		    seq_str => scalar(reverse $orig_t_subseq_str) }
		   );
    # Add unspliced target sequences
    foreach my $b (@borders) {
	...
	next if(length($q_seq_str_before_border) < $word_size);
	    ...
	    my $t_subseq_start = $t_subseq_end - length($q_seq_str) - 14;
	    $t_subseq_start = 1 if($t_subseq_start < 1);
	    $t_subseq_str = uc($t_seq->subseq($t_subseq_start, $t_subseq_end))
		.$t_subseq_str;
	    # add to array of sequences
	    push @targets, { d => $first_border - $b,
			     border2 => $word_hit,
			     seq_str => scalar(reverse $t_subseq_str) };
        }
    }
    return length($q_seq_str) if (@targets == 1);
...
sub _align_and_select_best
{
	...
	my ($score, $aln) = $self->_dp_align2($q_seq_str, $t->{seq_str});
...
#  use strings instead of arrays for seqs. get each char my substr($str, 0, 1, "");
#  inline subroutine _max_element
#  predefine sizes of @M and @N
#  possibly stop early
...
sub _dp_align {
	...
	foreach my $j (1..@T-1) {
	    ...
	    my @scores = ($M[$i-1][$j-1] + $p,
			  $M[$i-1][$j] + $gap_score,
			  $M[$i][$j-1] + $gap_score);
	    my $k = $self->_max_element(@scores);
	    $M[$i][$j] = $scores[$k];
	    $N[$i][$j] = $k;
	    ($best_i, $best_j) = ($i, $j)
		if ($M[$best_i][$best_j] < $M[$i][$j]);
	}
    }
...
sub _dp_align2 {
    ...
    # Align (i.e. fill in score and traceback matrices)
    foreach my $i (0..$Q_len) { $M[$i][0] = $gap_score * $i; }
    foreach my $j (0..$T_len) { $M[0][$j] = 0; }
    my $i = 1;
    while(my $q = substr($Q, 0, 1, "")) {
	my $j = 1;
	#my $T_tmp = $T;
	while(my $t = substr($T, $j-1, 1)) {
	    #print STDERR " $q$t";
	    my $p =  ($q eq $t) ? $match_score : $mismatch_score;
	    my @scores = ($M[$i-1][$j-1] + $p,
			  $M[$i-1][$j] + $gap_score,
			  $M[$i][$j-1] + $gap_score);
	    my $k = $self->_max_element(@scores);
...
#  return the number of bases of query seq after the first border.
{
    ...
    # Align; find best target
    my ($hsps, $target) = $self->_align_and_select_best0(\@q_seq_ary, \@targets);
