...
=head2 new
 ...
 Args      : exclute_on_revcom_ss  If true, excludes mappings with
	                           more revcomplemented canonical than
...
sub new
{
    ...
    my $self = bless {
	spliced_EST_score => ($args{spliced_EST_score} || 10),
	unspliced_EST_score => ($args{unspliced_EST_score} || 10),
	...
	bad_libs => ($args{bad_libs} || {}), # do $machine->bad_libs({22 => 1}) to give all sequences from lib #22 a score of 0
...
sub _score_mapping
{
    ...
    # calc score1
    my ($score1, $score2);
    my $library = ($m->primary_mapping_list)[0]->mRNAInfo->library;
    if($self->{bad_libs}->{$library}) {
	$score1 = $score2 = 0;
    }
    elsif(my @primary_m = $m->primary_mRNA_mapping_list) {
        my $is_priority_seq = 0;
	my ($mRNA_score, $refSeq_score) = (0,0);
        foreach my $primary_m (@primary_m) {
            if($primary_m->qName =~ /^N[MR]_/) {
        	    $refSeq_score =
		...
		$is_priority_seq = 1 if($self->{good_mRNA_libs}->{$library});
	    }
        }
	if($is_priority_seq) {
	    $score1 = $score2 = $self->high_mRNA_score + $refSeq_score;
	}
	else {
	    $score2 = $mRNA_score + $refSeq_score;
	    $score1 = $m->internally_primed ? 0 : $score2;
	}
    }
    elsif($m->primary_EST_mapping_list) {
	$score2 = $m->nr_introns ? $self->spliced_EST_score : $self->unspliced_EST_score;
        $score1 = $m->internally_primed ? 0 : $score2;
    }
    else {
    ...
    print STDERR "Scorer:", $m->acc_string, "\tscore1=$score1; score2=$score2\n" if($self->debug);
}
...
sub _calc_mRNA_mapping_score {
    ...
    my $high_score = $self->high_mRNA_score;
    my $max_offset = 16;
    my $min_utr = 20;

    return $high_score if($low_score == $high_score);
